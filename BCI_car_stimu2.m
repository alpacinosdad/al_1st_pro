...
%             end
       if wrct==[0 0 1920 1080]
           Screen('TextSize', w,170);
            while  ~KbCheck   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%开始闪烁
                Screen('FillRect',w,flicker(8.22,FrameNum,0.0*pi,ifi),[(wrct(3)-side)/2,40,(wrct(3)-side)/2+side,40+side]); 
                Screen('FillRect',w,flicker(10.08,FrameNum,0.5*pi,ifi),[(wrct(3)-side)/2,wrct(4)-40-side,(wrct(3)-side)/2+side,wrct(4)-40]); 
                Screen('FillRect',w,flicker(12.08,FrameNum,1.0*pi,ifi),[40,(wrct(4)-side)/2,40+side,(wrct(4)-side)/2+side]); 
                Screen('FillRect',w,flicker(11.12,FrameNum,1.5*pi,ifi),[wrct(3)-40-side,(wrct(4)-side)/2,wrct(3)-40,(wrct(4)-side)/2+side]); 
